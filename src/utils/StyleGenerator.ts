...
export const getDistrictStyle = (feature: any) => {
  ...
  if (!districtColors[osmId]) {
    // rgb ngẫu nhiên
    districtColors[osmId] = `rgba(${Math.floor(
      Math.random() * 256
...
export const getTreeStyle = (feature: any) => {
  const name: string = feature.get("sottcay"); // Tên cây (hoặc thông tin bạn muốn hiển thị)

  return new Style({
    image: new RegularShape({
      points: 3, // Số điểm của hình tam giác (3 điểm cho tam giác)
      radius: 15, // Bán kính của tam giác
      angle: Math.PI / 1.5, // Góc xoay để tam giác hướng lên trên
      fill: new Fill({ color: "#2ecc71" }), // Màu của hình tam giác (cây)
      stroke: new Stroke({ color: "#fff", width: 2 }), // Màu viền của tam giác
    }),
    fill: new Fill({
      color: "rgba(46, 204, 113, 0.25)", // Màu nền của cây
    }),
    stroke: new Stroke({
      color: "#2ecc71", // Màu viền cây
      width: 3, // Độ dày của viền
      lineCap: "round", // Kiểu đường viền
    }),
    text: new Text({
      text: name, // Dùng thông tin tên cây hoặc thông tin bạn muốn hiển thị
      font: "bold 10px Arial", // Cỡ chữ và kiểu font
      fill: new Fill({ color: "#000" }), // Màu chữ
      stroke: new Stroke({ color: "#fff", width: 2 }), // Màu viền chữ
      overflow: true, // Cho phép hiển thị text vượt ra ngoài vùng giới hạn
      padding: [10, 5, 5, 5], // Thêm padding cho text
      textBaseline: "middle", // Căn chỉnh chữ theo chiều dọc
      offsetY: 12, // Đẩy chữ xuống dưới tam giác
    }),
  });
};
